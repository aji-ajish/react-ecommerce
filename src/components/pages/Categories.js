    ...
    return (
        ...
        <>{categories.map((category => (
            <div key={category.id} className="w-4/5 sm:w-1/2 md:w-1/3 xl:w-1/4 p-4 md:h-96 ">
                <Link to={`/category/${category.id}`}  className="cursor-pointer block bg-gray-50 dark:bg-slate-900 transition-all duration-300 shadow-md hover:shadow-2xl rounded-lg overflow-hidden h-full border border-green-300 dark:border-slate-50 hover:scale-105">
